...
import {
  handleIncomingMessage as handleIncomingMessageOriginal,
  handleStatusUpdate as handleStatusUpdateOriginal,
...
app.use(
  session({
    secret:
      ...
      "whatsdesk-install-secret-key-change-in-production",
    resave: false,
...
const captureRawBody = (req, res, next) => {
  if (
    ...
    req.originalUrl === "/api/webhook/razorpay"
  ) {
    let data = "";
...
app.get("/api/is-demo-mode", async (req, res) => {
  try {
    const setting = await Setting.findOne({})
      .select(
        "is_demo_mode demo_user_email demo_user_password demo_agent_email logo_light_url logo_dark_url favicon_url demo_agent_password -_id",
      )
      .lean();
