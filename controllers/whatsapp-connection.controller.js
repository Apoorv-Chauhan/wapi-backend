...
const validateConnectionFields = (fields) => {
  ...
  if (!phoneNumberId || !accessToken || !whatsappBusinessAccountId) {
    return {
      isValid: false,
      message:
        "Phone number ID, access token, and WhatsApp Business Account ID are required",
    };
  }
...
const sanitizeConnectionData = (data) => {
  ...
  if (data.whatsappBusinessAccountId) {
    sanitized.whatsapp_business_account_id =
      data.whatsappBusinessAccountId.trim();
  }
...
const buildWhatsAppPayload = (params) => {
  const { recipientNumber, messageType, messageText, mediaId, fileName } =
    params;
...
//       whatsapp_business_account_id: whatsappBusinessAccountId,
//       app_id: appId,
//       registred_phone_number: registredPhoneNumber,
//       name,
//     } = req.body;
...
export const sendMessage = async (req, res) => {
  try {
    ...
    if (!whatsappConnection) {
      return res.status(404).json({
        message:
          "WhatsApp connection not found. Please configure your connection first.",
      });
    }
